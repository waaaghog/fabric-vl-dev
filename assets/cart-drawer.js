...
class CartDrawerComponent extends DialogComponent {
  ...
  #handlePopState = async () => {
    if (this.refs.dialog?.open) {
      this.refs.dialog.style.setProperty('--drawer-closing-animation', 'none');
      await this.closeDialog();
      this.refs.dialog.style.removeProperty('--drawer-closing-animation');
    }
  };
  ...
  #handleCartAdd = (event) => {
    if (this.hasAttribute('auto-open')) {
      ...
      // Only open if confirmed to be from product page; skip for cards and unknown sources
      if (target?.closest('.product-information')) {
        this.showDialog();
      }
    }
